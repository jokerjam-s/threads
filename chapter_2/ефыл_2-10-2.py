...
def sum_letters(param):
    result = 0
    no, sentence = param
    for char in sentence.lower():
        result += letter_values.get(char, 0)
    return no, result
...
def main():
    ...
    with ThreadPoolExecutor() as executor:
        futures = executor.map(sum_letters, enumerate(messages))

    result = sorted([f for f in futures], key=lambda x: x[1], reverse=True)
    print(messages[result[0][0]])
